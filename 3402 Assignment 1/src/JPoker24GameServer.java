...
public class JPoker24GameServer extends UnicastRemoteObject implements GameServer{
	...
	public HashMap<Integer, String> userProfileService(String user_name) throws RemoteException{
		...
		try{
			...
			if(!(item=Float.toString(result.getFloat(5))).isEmpty()){
				// Change the precision of float number
				float f = result.getFloat(5);
				BigDecimal b = new BigDecimal(f);
				item = Float.toString(b.setScale(1, BigDecimal.ROUND_HALF_UP).floatValue());
	...
	/* Register service: avoid duplicating user name and login user
	 * return:	1: success
	 * 			2: duplicating name
	 * 			-2: unknown error
	 */
		...
		try{
			...
			if(rs.next()){
				...
				for(int i=0; i<winningRecords; ++i){
					...
					// change the precision of float number
					BigDecimal b = new BigDecimal(winTime);
					winTime = b.setScale(1, BigDecimal.ROUND_HALF_UP).floatValue();
